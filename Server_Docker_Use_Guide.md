...
서버가 멈추거나 부팅이 안 되는 사고를 방지하기 위해 다음 5가지를 금지/권장합니다.
...
**5.가상환경(Virtual Environment) 적극 활용하기 (중요)**

랩실 서버는 공용이므로 시스템 전체(sudo pip install)에 라이브러리를 깔면, 나중에 다른 사람의 프로젝트나 시스템 업데이트와 버전 충돌이 날 수 있습니다.

따라서, 가급적이면 작업 디렉토리 폴더 안에 '가짜 파이썬 방' (가상 환경)을 구성해서 작업할것을 권장드립니다.

가상환경 구성 예시 코드는 다음과 같습니다.

```
# 1. 가상환경 생성 (이름은 venv)
python3 -m venv venv

# 2. 가상환경 진입 (이 명령어를 치면 프롬프트 앞에 (venv)가 생깁니다)
